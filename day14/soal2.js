...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < listBarang.length; i++) {
    ...
    // Iterate through the shoppers
    for (let j = 0; j < shoppers.length; j++) {
      // Check if the shopper wants to buy the current product and there is enough stock
      if (shoppers[j].product === listBarang[i][0] && productInfo.leftOver >= shoppers[j].amount) {
        // Update the product info
        productInfo.shoppers.push(shoppers[j].name);
        productInfo.leftOver -= shoppers[j].amount;
        productInfo.totalProfit += listBarang[i][1] * shoppers[j].amount;
      }
    }

    // Push the product info to the result array
