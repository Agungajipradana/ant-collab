...
function shoppingTime(memberId, money) {
  ...
  // Create an object to store the shopping information
  const shoppingInfo = {
    memberId: memberId || "Mohon maaf, toko X hanya berlaku untuk member saja",
    money: money,
    listPurchased: [],
    changeMoney: 0,
  };

  // Check if the memberId is provided and the money is sufficient
  if (!memberId) {
  ...
  // Iterate through the sorted products and add them to the listPurchased
  for (const product of products) {
    if (money >= product.price) {
      shoppingInfo.listPurchased.push(product.name);
      money -= product.price;
    }
  }

  // Set the remaining money as changeMoney
  shoppingInfo.changeMoney = money;
